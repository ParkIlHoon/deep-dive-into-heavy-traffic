...
class TimelineAdapter(
    ...
    override fun deleteAllByPostId(postId: UUID) = timelineRepository.deleteAllByPostId(postId)

    override fun deleteAllByMemberId(memberId: UUID) = timelineRepository.deleteAllByMemberId(memberId)

    @Transactional
    override fun deleteAllByPostIdIn(postIds: List<UUID>) = timelineRepository.deleteAllByPostIdIn(postIds)

    @Transactional
    override fun deleteTimelines(deleteTargets: List<TimelineDto.DeleteRequest>) {
        timelineClient.delete(deleteTargets)
    }
}
